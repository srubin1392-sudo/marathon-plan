exports.handler = async (event) => {
  ...
  const TRAINING_START_EPOCH = Math.floor(TRAINING_START.getTime() / 1000);

  // Fetch only runs since training start (Apr 27)
  let activities = [];
  let page = 1;
  while (true) {
    const res = await fetch(`https://www.strava.com/api/v3/athlete/activities?per_page=100&page=${page}&after=${TRAINING_START_EPOCH}`, {
  ...
  // Weekly mileage based on training runs only
  const weeklyMileage = {};
  runs.forEach(run => {
  ...
  return { statusCode: 200, headers, body: JSON.stringify({ runs, allRuns, weeklyMileage }) };
};
